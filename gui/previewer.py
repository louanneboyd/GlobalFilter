...
class GUI_MultiImagePreviewer(Frame):
    def __init__(self, parent, image_sources, data_type, max_width=None, max_height=None):
        ...
        self.image_frame = GUI_SingleImage(root, image_sources[0], data_type, max_width, max_height)
...
class GUI_SingleImage(Frame):
    ...
    def set(self, image_source, data_type):
        if (data_type == TYPE_ARRAY):
            self.image = image_from_array(image_source, self.max_width, self.max_height) # save a ref to the image so the garbage collector doesn't delete the image
        elif (data_type == TYPE_FILEPATH):
            self.image = image_from_file(image_source, self.max_width, self.max_height)
        else:
            raise Exception("Please specify the data_type, i.e., the format that image is in. (TYPE_ARRAY, TYPE_FILEPATH, ...)")
...
GUI_MultiImagePreviewer(root, [path, path2, path3], data_type = TYPE_FILEPATH, max_width="4i", max_height="4i").pack()

# GUI_SingleImage(root, cv2_image, data_type = TYPE_ARRAY, max_width="2i", max_height="2i").pack()
# GUI_SingleImage(root, image_source=path, data_type=TYPE_FILEPATH, max_width="1i", max_height="1i").pack()
